    ...
    useEffect(() => {
        const allRecords = async () => {
            ...
            try {
                ...
                const records = await contract.methods.displaySales().call({ from: account });
                // console.log(records);
    ...
    <div style={{
        ...
        <div>
            ...
            style={{
                ...
                <h1 className="h_name" style={{ marginTop: '-2rem', marginBottom: '2rem' }}>YOUR <span> SALES</span></h1>
                ...
                <div className="file-card" >
                    <Link className="file-card-icon image" to='/create' style={{fontSize:'1.2rem', fontWeight:'bold'}}>
                        
                        CREATE
                    </Link>
                    <Link className="file-card-icon video" to='/update' style={{fontSize:'1.2rem', fontWeight:'bold'}}>
                        
                        UPDATE
                    </Link>
                    <Link className="file-card-icon audio" to='/delete' style={{fontSize:'1.2rem', fontWeight:'bold'}}>
                ...
                .map((record) => {
                    return (
                        <>
                            ...
                            <div className="display_all_records_card change_card" key={record.id} style={record.id !== '' && record.title !== '' && record.price !== '' && record.quantity !== '' ? {} : { display: "none" }}>
                            {/* <Link  style={{width:"100%"}}> */}
                                <a href={record.url} target="_blank" style={{color:"rgb(57, 56, 56)",fontSize:"0.8rem",fontWeight:"500",width:"100%"}}>
                                    ...
                                    <div style={{display:"flex", flexDirection:"row", justifyContent:"space-between"}}>
                                        <Link  to="/update" style={{width:"100%", cursor:"pointer"}}>
                                        {/* <Link  to="/create" style={{width:"100%",display:"flex",justifyContent:"center"}}> */}
                                            <div style={{zIndex:"100",background:"red",color:"white",width:"50%",display:"flex",justifyContent:"center",borderRadius:"1rem"}}>UPDATE</div>
                                        
                                        </Link>
                                        <Link  to="/delete" style={{width:"100%", cursor:"pointer"}}>
                                        
                                            <div style={{zIndex:"100",background:"red",color:"white",width:"50%",display:"flex",justifyContent:"center",borderRadius:"1rem"}}>DELETE</div>
